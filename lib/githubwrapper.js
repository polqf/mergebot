...
GithubWrapper.prototype.commentOnPullRequest = function(prNumber, message, callback) {
    this.removeBotOldMessages(prNumber)
...
GithubWrapper.prototype.removeBotOldMessages = function(prNumber) {
    Logger.log("Removing old comments on #" + prNumber)
    if (global.debug) {
        return
    }
    ...
    this.gitHubApi.issues.getComments({
        ...
        result.data.forEach(function(comment) {
            if (comment.user.login.toLowerCase() != process.env.GITHUB_BOT_USER.toLowerCase()) { return; }
            if (comment.body.includes("<!--")) { return; } // Danger comment
            self.removeComment(comment.id)
        })
    })
}

GithubWrapper.prototype.removeComment = function(commentId) {
    this.gitHubApi.issues.deleteComment({
        owner: this.repoOwner,
        repo: this.repoName,
        id: commentId
    },  function(error, result) {
