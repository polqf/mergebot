...
MergeBot.prototype.mergeMethodFor = function(originalMessage) {
	...
	var user = process.env.GITHUB_BOT_USER.toLowerCase()
...
MergeBot.prototype.notifyMergeability = function(githubWrapper, prNumber, callback) {
	...
	githubWrapper.reviewReport(prNumber, function(reviews) {
		...
		var digest = MergeBot.reviewDigestFrom(reviews, self.prAuthor)
		var message = digest.message
		var approved = (digest.approved == null) ? true : digest.approved

		Logger.log("PR " + prNumber + (approved ? " is " : " is not ") + "approved")
		...
		var webhook = new SlackWebhook()
		webhook.sendMessage(process.env.SLACK_BOT_TOKEN, "Missing PR Review", self.prName + " - #" + prNumber, self.prURL, reviewsCount, message)
	})
}

MergeBot.emojiForState = function(state) {
	switch(state) {
		case "approved":
...
MergeBot.prototype.postAlertMessage = function(githubWrapper, prNumber, callback) {
	var message = process.env.GITHUB_ALERT_MESSAGE
...
MergeBot.reviewDigestFrom = function(reviews, author) {
	var message = ""
	var approved = null
	Object.keys(reviews).forEach(function(user) {
		...
		var approvedReview = value == "approved"
		approved = (approved == null) ? approvedReview : approved && approvedReview
		message = message + "@" + user + "  " + MergeBot.emojiForState(value) + "\n"
	})
